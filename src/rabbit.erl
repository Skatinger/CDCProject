...
%%  Size = 1 + find_grass(neighbours),
  ...
  receive %Pid is the pid of the desired field, so that the rabbit can register itself on it
    ...
    {rabbit, {Index, Pid}} -> io:format("???? ~p~n", [self()]), rabbit(MyIndex, {State, Size - 1, Age + 1}, RabbitControllerPid); %mate?
    ...
    {[], {Index, Pid}} ->
      ...
      receive
        ...
        _ -> io:format("============ WTF ========~n"), rabbit(MyIndex, {State, Size -1, Age + 1}, RabbitControllerPid)
      end;
